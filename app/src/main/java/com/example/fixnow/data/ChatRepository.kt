...
data class MensajeDB(
    ...
    @SerialName("id_receptor") val idReceptor: String,
...
object ChatRepository {
    ...
    suspend fun enviarMensaje(emisorId: String, receptorId: String, texto: String) {
        val mensaje = MensajeDB(idEmisor = emisorId, idReceptor = receptorId, contenido = texto, leido = false)
    ...
    suspend fun obtenerMensajesHistoricos(miId: String, otroId: String): List<MensajeDB> {
        return try {
            client.postgrest["mensajes"].select {
                filter {
                    or {
                        and { eq("id_emisor", miId); eq("id_receptor", otroId) }
                        and { eq("id_emisor", otroId); eq("id_receptor", miId) }
                    }
                }
    ...
    suspend fun marcarComoLeidos(miId: String, otroId: String) {
        try {
            client.postgrest["mensajes"].update({ set("leido", true) }) {
                filter { eq("id_emisor", otroId); eq("id_receptor", miId); eq("leido", false) }
            }
        } catch (e: Exception) { }
    }
    ...
    fun escucharTotalMensajesNoLeidos(miId: String): Flow<Int> {
        return client.postgrest["mensajes"]
            .selectAsFlow(primaryKey = MensajeDB::id)
            .map { lista -> lista.count { it.idReceptor == miId && !it.leido } }
    }

    suspend fun obtenerConversaciones(miId: String): List<String> {
        return try {
            val mensajes = client.postgrest["mensajes"].select {
                filter { or { eq("id_emisor", miId); eq("id_receptor", miId) } }
            }.decodeList<MensajeDB>()
            mensajes.map { if (it.idEmisor == miId) it.idReceptor else it.idEmisor }.distinct()
        } catch (e: Exception) { emptyList() }
    }
}

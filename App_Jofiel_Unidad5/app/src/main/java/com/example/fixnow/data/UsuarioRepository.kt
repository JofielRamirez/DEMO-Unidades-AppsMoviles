...
data class Cita(
    val id: String? = null,
    @SerialName("id_cliente") val idCliente: String = "",
    @SerialName("id_socio") val idSocio: String = "",
    val fecha: String = "",
    val estado: String = "pendiente",
    val detalles: String? = null,
    @SerialName("lat_cliente") val latCliente: Double? = null,
    @SerialName("lon_cliente") val lonCliente: Double? = null
)
...
object UsuarioRepository {
    ...
    suspend fun convertirseEnPrestador(uid: String, tipo: String, urlId: String? = null, urlAnt: String? = null) {
        val uidLimpio = uid.replace("\"", "").trim()
        client.postgrest["Usuarios"].update({
            set("es_prestador", true); set("tipo_servicio", tipo)
    ...
    suspend fun actualizarUbicacion(uid: String, lat: Double, lon: Double) {
        val uidLimpio = uid.replace("\"", "").trim()
        try {
            client.postgrest["Usuarios"].update({ set("latitud", lat); set("longitud", lon) }) {
                filter { eq("id", uidLimpio) }
            }
        } catch (e: Exception) { Log.e("REPO", "Error actualizando ubicacion: ${e.message}") }
    }

    suspend fun actualizarDisponibilidad(uid: String, disponible: Boolean) {
        val uidLimpio = uid.replace("\"", "").trim()
        try {
            client.postgrest["Usuarios"].update({ set("disponible", disponible) }) {
                filter { eq("id", uidLimpio) }
            }
        } catch (e: Exception) { Log.e("REPO", "Error actualizando disponibilidad: ${e.message}") }
    }
    ...
    suspend fun subirFotoPerfil(uid: String, data: ByteArray): String {
        ...
        bucket.upload(path = fileName, data = data) { upsert = true }
    ...
    suspend fun subirDocumentoSocio(uid: String, fileName: String, data: ByteArray, bucketName: String): String {
        return try {
            ...
            bucket.upload(path = path, data = data) { upsert = true }
    ...
    suspend fun obtenerSociosPorCategoria(categoria: String): List<UsuarioPerfil> {
        return try {
            client.postgrest["Usuarios"].select {
                filter { eq("es_prestador", true); ilike("tipo_servicio", "%$categoria%") }
    ...
    suspend fun obtenerSociosDisponiblesPorCategoria(categoria: String): List<UsuarioPerfil> {
        return try {
            client.postgrest["Usuarios"].select {
                filter { eq("es_prestador", true); eq("disponible", true); ilike("tipo_servicio", "%$categoria%") }
            }.decodeList<UsuarioPerfil>()
        } catch (e: Exception) { emptyList() }
    }
    ...
    suspend fun crearCita(idCliente: String, idSocio: String, fecha: String, detalles: String, lat: Double? = null, lon: Double? = null): Cita? {
        val nuevaCita = Cita(idCliente = idCliente, idSocio = idSocio, fecha = fecha, estado = "pendiente", detalles = detalles, latCliente = lat, lonCliente = lon)
        return try {
            client.postgrest["citas"].insert(nuevaCita) { select() }.decodeSingleOrNull<Cita>()
        } catch (e: Exception) { null }
    }

    suspend fun obtenerCitaPorId(citaId: String): Cita? {
        return try { client.postgrest["citas"].select { filter { eq("id", citaId) } }.decodeSingleOrNull<Cita>() }
        catch (e: Exception) { null }
    }

    suspend fun obtenerCitasSocio(uid: String): List<Cita> {
        return try { client.postgrest["citas"].select { filter { eq("id_socio", uid) } }.decodeList<Cita>() }
        catch (e: Exception) { emptyList() }
    }

    suspend fun obtenerCitasCliente(uid: String): List<Cita> {
        return try { client.postgrest["citas"].select { filter { eq("id_cliente", uid) } }.decodeList<Cita>() }
        catch (e: Exception) { emptyList() }
    }

    suspend fun actualizarEstadoCita(citaId: String, nuevoEstado: String) {
        client.postgrest["citas"].update({ set("estado", nuevoEstado) }) { filter { eq("id", citaId) } }
    }
}
